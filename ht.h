...
public:
    ...
    HASH_INDEX_T next() 
    {
      //returns npos when reaching m probes
      if(this->numProbes_ >= this->m_){
        return this->npos;
      }
      

      //double hash implemented( s +i *step) %m
...
HashTable<K,V,Prober,Hash,KEqual>::~HashTable()
{
  ...
  //destructor delted allocated hashitems
...
{
  size_t totalSlots =0;
  // will see if resizing neede to occur
  //loadfactor  should incude both the deelted and not deleted items
  for(size_t i =0; i< table_.size(); i++){
    //empty case
    if(table_[i] != nullptr){
      totalSlots++;
  ...
  double loadF = (double)totalSlots / CAPACITIES[mIndex_];
  if(loadF >= resizeAlpha_){
    resize();
  }
  ...
  // insert a new item when the location is empty
  // the if a key exists we must update the item
  // the saftey check
...
{
  size_t numCapp = sizeof(CAPACITIES) / sizeof(HASH_INDEX_T);
  //checking if we need to resize 
  if(mIndex_ + 1 >= numCapp){
    throw std::logic_error("No Cappacities available");
  }
  ...
  // will rehash all the items that were nto delted 
  for(size_t i=0; i<oldTable.size(); i++){
    if(oldTable[i] != nullptr){
      if(!oldTable[i]->deleted){
        insert(oldTable[i]->item);
      }
      //delted old hashtable
      delete oldTable[i];
    }
  }
